async function initHive() {
    ...
    // 1. Fetch Data
    const response = await fetch(`/api/graph/${ALIAS}?mode=macro&limit=2000`);
    ...
    svg.append("g")
        ...
        .on("mouseover", (event, d) => {
            ...
            // Highlight links
            d3.selectAll(".link")
                .style("stroke-opacity", l => (l.source === d.did || l.target === d.did) ? 0.8 : 0.05)
                .style("stroke-width", l => (l.source === d.did || l.target === d.did) ? 2 : 1);
        })
        .on("mouseout", () => {
